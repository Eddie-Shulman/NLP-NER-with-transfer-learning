...
def get_corpus_location(corpus_name):
    ...
    """
    config = ConfigParser.RawConfigParser()
    config.read(os.path.join(ABSOLUTE_PATH, 'file_locations.cfg'))
    ...
    filename_end = xx
    data_dir = os.path.join(ABSOLUTE_PATH, config.get(corpus_name, 'data_dir'))
    docs_dir = os.path.join(ABSOLUTE_PATH, config.get(corpus_name, 'docs_dir'))
